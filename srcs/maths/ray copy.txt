...
// 		add_vectors(&hit_point, &ray->start, &hit_point);
...
void	init_ray(t_ray *ray)
{
    ...
    // if (t_minus >= EPS) {
    //     hit_t  = t_minus;
    //     pair_t = (t_plus >= EPS) ? t_plus : INF; // second t en avant (sortie)
    // } else if (t_plus >= EPS) {
    //     // On est à l'intérieur : l'entrée est derrière, on sort en t_plus
    //     hit_t  = t_plus;
    //     pair_t = INF;
    // } else {
    //     // Les deux intersections sont derrière l'origine du rayon
    //     return;
    // }
...
void	IntersectRaySphere(t_ray *ray, t_sphere *sphere)
{
	...
	// if (!isinf(t1) && (isinf(ray->shape.t1) || t1 < ray->shape.t1) && t1 > 1) //comme avant mais pas sûr que ça soit parfaitement juste
	// {
	// 	ray->shape.t1 = t1;
	// 	ray->shape.shape = sphere;
	// 	ray->shape. = sphere;
	// }
	// if (!isinf(t2) && (isinf(ray->shape.t2) || t2 < ray->shape.t2) && t2 > 1)
	// {
	// 	ray->shape.t2 = t2;
	// 	ray->shape.shape = sphere;
	// }
