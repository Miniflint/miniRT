...
t_tri_lib	*tri_lib(void)
{
	static t_tri_lib	tri_lib = {
		.init = _init_tri_lib, .destroy = _destroy_tri_lib, .loop = _main_loop,
		.get_end_function = _get_end_func, .create_window = _create_window,
		.create_render = _create_render, .destroy_window = _destroy_window,
		.destroy_render = _destroy_render, .print_error = _print_error,
		.get_error_msg = _get_error_message, .open_img = _open_img,
		.get_pixel = _get_pixel, .put_pixel_to_render = _put_pixel_to_render,
		.quit = _quit, .replace_pixel_on_render = _replace_pixel_on_render,
		...
		.fps = 0.0, .fps_target = FPS_TARGET, .auto_draw = 0,
		._imgs = NULL, ._windows = NULL, ._mlx = NULL, .event = NULL,
		._user_content = NULL, ._bg_color = BACKGROUND_COLOR,
		._ernum = TRI_SUCCESS, ._user_end = NULL, .fps_max = FPS_MAX,
